...
from partisoproduct.models import MvpQuoteRequest, MvpQuotePartDetail, MvpQuoteProduct, Part1
...
class MvpQuotePartDetailSerializer(serializers.ModelSerializer):
...
class MvpQuoteProductDetailSerializer(serializers.ModelSerializer):
    part_details = MvpQuotePartDetailSerializer(many=True, read_only=True)
    modular_product_name = serializers.CharField(source='modular_product.name', read_only=True)
    
    class Meta:
        model = MvpQuoteProduct
        fields = [
...
class MvpQuoteProductWriteSerializer(serializers.ModelSerializer):
    part_details = MvpQuotePartDetailSerializer(many=True, required=False)
...
class HybridQuoteRequestSerializer(serializers.ModelSerializer):
    products = MvpQuoteProductDetailSerializer(many=True, read_only=True)

    class Meta:
        model = MvpQuoteRequest
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'view' in self.context and self.context['view'].action in ['create', 'calculate_quote']:
            self.fields['products'] = MvpQuoteProductWriteSerializer(many=True, required=True)

    def create(self, validated_data):
        products_data = validated_data.pop('products')
        
        quote_request = MvpQuoteRequest.objects.create(**validated_data)
        
        for product_data in products_data:
            part_details_data = product_data.pop('part_details', [])
            quote_product = MvpQuoteProduct.objects.create(quote=quote_request, **product_data)
            ...
            for part_detail_data in part_details_data:
                ...
                MvpQuotePartDetail.objects.create(
