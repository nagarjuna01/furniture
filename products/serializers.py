...
class PartSerializer(serializers.ModelSerializer):
    # --- For Reading (Output) ---
    material = WoodEnSerializer(read_only=True)
    top_edge_band = EdgeBandSerializer(read_only=True)
    left_edge_band = EdgeBandSerializer(read_only=True)
    bottom_edge_band = EdgeBandSerializer(read_only=True)
    right_edge_band = EdgeBandSerializer(read_only=True)
...
class StandardProductSerializer(serializers.ModelSerializer):
    ...
    brand = BrandSerializer(read_only=True) # For displaying nested Brand info
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'product_type', 'image', 'display_selling_price', 'slug',
                  'category', 'type', 'model', 'brand',  'meta_title', 'meta_description',
                  'created_at', 'updated_at']