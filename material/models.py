...
class EdgeBand(models.Model):
    edge_depth = models.DecimalField(max_digits=5, decimal_places=2, default=0) # Changed to DecimalField
    e_thickness = models.DecimalField(max_digits=5, decimal_places=2) # Changed to DecimalField
    ...
    # You might want a 'name' field for better identification in admin/UI
    name = models.CharField(max_length=255, blank=True, null=True, 
                            help_text="e.g., 'White 1mm', 'Wood Grain 2mm'")

    def clean(self):
        super().clean() # Call super's clean method if it exists
        ...
        if self.edge_depth is not None and self.edge_depth < 0:
            raise ValidationError("Edge depth cannot be negative.")
        if self.e_thickness is not None and self.e_thickness < 0:
            raise ValidationError("Edge band thickness cannot be negative.")

    @property
    def sl_price(self):
        # Ensure sl_price is calculated only if p_price is not None
        # and always returns a Decimal, quantized for currency.
        if self.p_price is not None:
            return (self.p_price * Decimal('1.2')).quantize(Decimal('0.01'))
        return Decimal('0.00')

    def __str__(self):
        # Include name if available, otherwise fallback to dimensions
        if self.name:
            return self.name
        return f"EdgeBand {self.edge_depth}mm X {self.e_thickness}mm"
...
class WoodEn(models.Model):
    ...
    compatible_edgebands = models.ManyToManyField('EdgeBand', blank=True, related_name='compatible_woodens')
